...
class Era2016:
    ...
    def _generate_customers(self):
        ...
        for c in self.universe.customers:
            ...
            # Extract integer from CUST_00123
            cust_id = int(c.internal_id.split('_')[1])
            data.append({
                "cust_id": cust_id,
    ...
    def _generate_orders(self):
        ...
        for o in self.universe.orders:
            # orders logic
            order_id = int(o.internal_id.split('_')[1])
            cust_id = int(o.customer_id.split('_')[1])
            data.append({
                "order_id": order_id,
                "cust_id": cust_id,
    ...
    def _generate_products(self):
        ...
        for p in self.universe.products:
            prod_id = int(p.internal_id.split('_')[1])
            data.append({
                "prod_id": prod_id,
                "title": p.name,
                "category": p.category,
